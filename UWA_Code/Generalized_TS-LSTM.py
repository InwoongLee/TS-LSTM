...
myFolderPath = './TrialSaver/%s/%04d%02d%02d_%02d%02d_BD_%d_KB_%.2f_TrV_%01d_%01d_TeV_%01d_%01d_NS_%02d'%(view_subject,_now.year,
                                                                           _now.month,
                                                                           _now.day,
                                                                           _now.hour,
                                                                           _now.minute,
                                                                           BatchDivider,
                                                                           myKeepProb, train_set[0], train_set[1], test_set1[0], test_set2[0], N_S)
os.mkdir(myFolderPath)



# win_size_data = [21, 21, 21, 21, 21,
#                  42, 42, 42,
#                  63,
#                  84, 84,
#                  126,
#                  167]
# stride_data = [21, 21, 21, 21, 21,
#                42, 42, 42,
#                52,
#                84, 84,
#                41,
#                100]
# start_time_data = [1, 2, 3, 6, 7,
#                    2, 3, 6,
#                    2,
#                    8, 9,
#                    7,
...
# N_S = 12
# win_size_data = [21, 21, 21, 21,
#                  42, 42, 42, 42,
#                  63, 63,
#                  105,
#                  167]
# stride_data = [21, 21, 21, 21,
#                42, 42, 42, 42,
#                52, 52,
#                62,
#                100]
# start_time_data = [1, 2, 3, 6,
#                    2, 3, 6, 8,
#                    8, 9,
#                    7,
#                    5]
...
# N_S = 9
# win_size_data = [21, 21, 21,
#                  42, 42, 42, 42,
#                  63,
#                  105]
# stride_data = [21, 21, 21,
#                42, 42, 42, 42,
#                52,
#                62]
# start_time_data = [1, 3, 6,
#                    2, 3, 6, 8,
#                    9,
#                    7]
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session(config=sessConfig) as session:
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest2 = MP_runner(is_training=False, config=eval_config2, labels=AS_test_label2, win_sizes=win_size, strides = stride, start_times = start_time)
            ...
            for i in range(config.max_max_epoch):
                ...
                if i % 10 == 0:
                    ...
                    print("Epoch: %d Learning rate: %.6f Train Accuracy_AM: %.4f Train Accuracy_GM: %.4f" % (
                    i, session.run(m.lr), train_accuracy, train_accuracy_geo))
                    ...
                    csvWriter.writerow(
                        [strtime, i, (end_epoch - stt_epoch), train_accuracy, train_accuracy_geo, train_cost])

                if i % 100 == 0:
                    test_cost_L, test_cost, test_accuracy, test_accuracy_geo, te_p_l, te_p_l_geo, te_g_l = run_epoch(
                        session,
                        mtest,
                        feature_test,
                        AS_test_label,
                        tf.no_op(), is_training=False)
                    test_cost_L2, test_cost2, test_accuracy2, test_accuracy_geo2, te_p_l2, te_p_l_geo2, te_g_l2 = run_epoch(
                        session,
                        mtest2,
                        feature_test2,
                        AS_test_label2,
                    ...
                    print("Test Accuracy_AM: %.5f Test Accuracy_GM : %.5f\n" % (test_accuracy2, test_accuracy_geo2))
                    csvWriter.writerow(["Test Accuracy_AM :", test_accuracy2, "Test Accuracy_GM :", test_accuracy_geo2])
                    ...
                    for j in range(config.class_size):
                        class_prob_geo[j] = confusion_matrix_geo[j][j] / np.sum(
                            confusion_matrix_geo[j][0:config.class_size])
                    for j in range(config.class_size):
                    ...
                    confusion_matrix_geo2 = np.zeros([config.class_size, config.class_size + 1])
                    class_prob_geo2 = np.zeros([config.class_size])
                    for j in range(len(te_g_l2)):
                        confusion_matrix_geo2[te_g_l2[j]][te_p_l_geo2[j]] += 1
                    for j in range(config.class_size):
                        class_prob_geo2[j] = confusion_matrix_geo2[j][j] / np.sum(
                            confusion_matrix_geo2[j][0:config.class_size])
                    for j in range(config.class_size):
                        confusion_matrix2[j][config.class_size] = class_prob2[j]
                        confusion_matrix_geo2[j][config.class_size] = class_prob_geo2[j]

                    with open(folder_path + "/view-test-AM2-" + str(i) + ".csv", "w") as csvfile2:
                        csvwriter22 = csv.writer(csvfile2)
                        for j in range(config.class_size):
                            csvwriter22.writerow(confusion_matrix2[j])
                    with open(folder_path + "/view-test-GM2-" + str(i) + ".csv", "w") as csvfile_geo2:
                        csvwriter2_geo2 = csv.writer(csvfile_geo2)
                        for j in range(config.class_size):
                            csvwriter2_geo2.writerow(confusion_matrix_geo2[j])
