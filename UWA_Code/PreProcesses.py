    ...
    for batchNo in range(feature.shape[0]):
        ...
        for frmNo in range(feature.shape[1]):
            if (np.linalg.norm(feature[batchNo][frmNo]) != 0) and (first_index == False):
                new_origin = (feature[batchNo][frmNo][27:30] + feature[batchNo][frmNo][36:39]) / 2
                # v1 = feature[batchNo][frmNo][3:6] - feature[batchNo][frmNo][6:9]
                v1 = [0, 1, 0]
                v1 = v1 / np.linalg.norm(v1)
                u2 = feature[batchNo][frmNo][27:30] - feature[batchNo][frmNo][36:39]
                # print(u2)
                ...
                for nodeNo in range(int(feature.shape[2] / 3)):
