...
from PreProcesses import body_rotation, bone_length, extract_AS, extract_data_label, extract_feature, count_zeros, feature_only_diff_0, one_hot_labeling
    ...
    def __init__(self, is_training, config, labels):
        ...
        with tf.device(sw_0):
            with tf.name_scope('%s_%d' % ('mGPU', 0)) as scope:
                ...
                self.depthconcat = mL.get_depth_concat_output()
...
class TS_LSTM_Long_0(object):
    def __init__(self, is_training, config, input_data):
        ...
        with tf.variable_scope("Dep_Con_0"):
            ...
            # concat_output_real_0 = input_0
            # self.out_concat_output_real_0 = tf.reshape(concat_output_real_0,
            #                                            [batch_size, num_LSTMs_0 * num_steps * size])
...
def main(_):
    ...
    feature_train = body_rotation(train_sklt1)
    ...
    with tf.Graph().as_default(), tf.Session(config=sessConfig) as session:
        with tf.device('/cpu:0'):
            ...
            for i in range(config.max_max_epoch):
                ...
                if i % 100 == 0:
                    test_cost_L, test_cost, test_accuracy, te_p_l, te_g_l = run_epoch(session, mtest, feature_test,
                                                                                      AS_test_label,
                                                                                      tf.no_op(),
                                                                                      is_training=False)
