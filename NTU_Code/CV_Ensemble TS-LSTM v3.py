...
class TS_LSTM_Long_1(object):
    def __init__(self, is_training, config, input_data):
         ...
         num_LSTMs_1 = len(range(0, num_steps - win_size_1 + start_time_1, stride_1))
         print range(0, num_steps - win_size_1 + start_time_1, stride_1)
         ...
         for time_step in range(num_steps):
             for win_step in range(num_LSTMs_1):
                 ...
                 LSTM_path = os.path.join('LSTM', str(win_step))
                 with tf.variable_scope(LSTM_path):
                     if time_step < win_step * stride_1:
                         cell_output = tf.zeros([batch_size, size])
                         self.outputs_1[win_step].append(cell_output)
                     elif time_step >= win_step * stride_1 and time_step < start_time_1 + win_step * stride_1 + win_size_1:
                         if time_step > win_step * stride_1: tf.get_variable_scope().reuse_variables()
                         if time_step < start_time_1 + win_step * stride_1:
                             distance = tf.reduce_sum(inputs[:, 0:time_step+1, :], 1) / (start_time_1 + 1)
                         else:
                             if start_time_1 == 1:
                                 distance = inputs[:, time_step, :] / (start_time_1 + 1)
                             else:
                                 distance = tf.reduce_sum(inputs[:, time_step - start_time_1:time_step+1, :], 1) / (
                                 start_time_1 + 1)
         ...
         with tf.variable_scope("Dep_Con_1"):
             ...
             for win_step in range(num_LSTMs_1):
                 ...
                 temp_output_1[win_step] = tf.reshape(output_1[win_step], [batch_size, num_steps, size])
                 if win_step == 0:
                     input_1 = temp_output_1[win_step]
                 else:
                     input_1 = tf.concat(1, [input_1, temp_output_1[win_step]])
             input_1 = tf.reshape(input_1, [batch_size, num_LSTMs_1, num_steps, size])
...
class TS_LSTM_Long_2(object):
    def __init__(self, is_training, config, input_data):
         ...
         num_LSTMs_2 = len(range(0, num_steps - win_size_2 + start_time_2, stride_2))
         print range(0, num_steps - win_size_2 + start_time_2, stride_2)
         ...
         for time_step in range( num_steps):
             for win_step in range(num_LSTMs_2):
                 ...
                 LSTM_path = os.path.join('LSTM', str(win_step))
                 with tf.variable_scope(LSTM_path):
                     if time_step < win_step * stride_2:
                         cell_output = tf.zeros([batch_size, size])
                         self.outputs_2[win_step].append(cell_output)
                     elif time_step >= win_step * stride_2 and time_step < start_time_2 + win_step * stride_2 + win_size_2:
                         if time_step > win_step * stride_2: tf.get_variable_scope().reuse_variables()
                         if time_step < start_time_2 + win_step * stride_2:
                             distance = tf.reduce_sum(inputs[:, 0:time_step+1, :], 1) / (start_time_2 + 1)
                         else:
                             if start_time_2 == 1:
                                 distance = inputs[:, time_step, :] / (start_time_2 + 1)
                             else:
                                 distance = tf.reduce_sum(inputs[:, time_step - start_time_2:time_step+1, :], 1) / (
                                 start_time_2 + 1)
         ...
         with tf.variable_scope("Dep_Con_2"):
             ...
             for win_step in range(num_LSTMs_2):
                 ...
                 temp_output_2[win_step] = tf.reshape(output_2[win_step], [batch_size, num_steps, size])
                 if win_step == 0:
                     input_2 = temp_output_2[win_step]
                 else:
                     input_2 = tf.concat(1, [input_2, temp_output_2[win_step]])
             input_2 = tf.reshape(input_2, [batch_size, num_LSTMs_2, num_steps, size])
...
class TS_LSTM_Medium_3(object):
    def __init__(self, is_training, config, input_data):
        ...
        num_LSTMs_3 = len(range(0, num_steps - win_size_3 + start_time_3, stride_3))
        print range(0, num_steps - win_size_3 + start_time_3, stride_3)
        ...
        for time_step in range(num_steps):
            for win_step in range(num_LSTMs_3):
                ...
                LSTM_path = os.path.join('LSTM', str(win_step))
                with tf.variable_scope(LSTM_path):
                    if time_step < win_step * stride_3:
                        cell_output = tf.zeros([batch_size, size])
                        self.outputs_3[win_step].append(cell_output)
                    elif time_step >= win_step * stride_3 and time_step < start_time_3 + win_step * stride_3 + win_size_3:
                        if time_step > win_step * stride_3: tf.get_variable_scope().reuse_variables()
                        if time_step < start_time_3 + win_step * stride_3:
                            distance = tf.reduce_sum(inputs[:, 0:time_step+1, :], 1) / (start_time_3 + 1)
                        else:
                            if start_time_3 == 1:
                                distance = inputs[:, time_step, :] / (start_time_3 + 1)
                            else:
                                distance = tf.reduce_sum(inputs[:, time_step - start_time_3:time_step+1, :], 1) / (start_time_3 + 1)
        ...
        with tf.variable_scope("Dep_Con_3"):
            ...
            for win_step in range(num_LSTMs_3):
                ...
                temp_output_3[win_step] = tf.reshape(output_3[win_step], [batch_size, num_steps, size])
                if win_step == 0:
                    input_3 = temp_output_3[win_step]
                else:
                    input_3 = tf.concat(1, [input_3, temp_output_3[win_step]])
            input_3 = tf.reshape(input_3, [batch_size, num_LSTMs_3, num_steps, size])
...
class TS_LSTM_Medium_4(object):
    def __init__(self, is_training, config, input_data):
        ...
        num_LSTMs_4 = len(range(0, num_steps - win_size_4 + start_time_4, stride_4))
        print range(0, num_steps - win_size_4 + start_time_4, stride_4)
        ...
        for time_step in range(num_steps):
            for win_step in range(num_LSTMs_4):
                ...
                LSTM_path = os.path.join('LSTM', str(win_step))
                with tf.variable_scope(LSTM_path):
                    if time_step < win_step * stride_4:
                        cell_output = tf.zeros([batch_size, size])
                        self.outputs_4[win_step].append(cell_output)
                    elif time_step >= win_step * stride_4 and time_step < start_time_4 + win_step * stride_4 + win_size_4:
                        if time_step > win_step * stride_4: tf.get_variable_scope().reuse_variables()
                        if time_step < start_time_4 + win_step * stride_4:
                            distance = tf.reduce_sum(inputs[:, 0:time_step+1, :], 1) / (start_time_4 + 1)
                        else:
                            if start_time_4 == 1:
                                distance = inputs[:, time_step, :] / (start_time_4 + 1)
                            else:
                                distance = tf.reduce_sum(inputs[:, time_step - start_time_4:time_step+1, :], 1) / (start_time_4 + 1)
        ...
        with tf.variable_scope("Dep_Con_4"):
            ...
            for win_step in range(num_LSTMs_4):
                ...
                temp_output_4[win_step] = tf.reshape(output_4[win_step], [batch_size, num_steps, size])
                if win_step == 0:
                    input_4 = temp_output_4[win_step]
                else:
                    input_4 = tf.concat(1, [input_4, temp_output_4[win_step]])
            input_4 = tf.reshape(input_4, [batch_size, num_LSTMs_4, num_steps, size])
...
class TS_LSTM_Short_5(object):
    def __init__(self, is_training, config, input_data):
        ...
        num_LSTMs_5 = len(range(0, num_steps - win_size_5 + start_time_5, stride_5))  # range ( start, end, delta), len = 5
        print range(0, num_steps - win_size_5 + start_time_5, stride_5)
        print("num_LSTMs_5: ", num_LSTMs_5)
        for time_step in range(num_steps):  # time_step is like i (1,76)
            for win_step in range(num_LSTMs_5):  # win_step (1,5)
                ...
                LSTM_path = os.path.join('LSTM', str(win_step))
                with tf.variable_scope(LSTM_path):
                    if time_step < win_step * stride_5:
                        cell_output = tf.zeros([batch_size, size])
                        self.outputs_5[win_step].append(cell_output)
                    elif time_step >= win_step * stride_5 and time_step < start_time_5 + win_step * stride_5 + win_size_5:
                        if time_step > win_step * stride_5: tf.get_variable_scope().reuse_variables()
                        if time_step < start_time_5 + win_step * stride_5:
                            distance = tf.reduce_sum(inputs[:, 0:time_step+1, :], 1) / (start_time_5 + 1)
                        else:
                            if start_time_5 == 1:
                                distance = inputs[:, time_step, :] / (start_time_5 + 1)
                            else:
                                distance = tf.reduce_sum(inputs[:, time_step - start_time_5:time_step+1, :], 1) / (start_time_5 + 1)
        ...
        with tf.variable_scope("Dep_Con_5"):
            ...
            for win_step in range(num_LSTMs_5):
                ...
                temp_output_5[win_step] = tf.reshape(output_5[win_step], [batch_size, num_steps, size])
                if win_step == 0:
                    input_5 = temp_output_5[win_step]
                else:
                    input_5 = tf.concat(1, [input_5, temp_output_5[win_step]])
            input_5 = tf.reshape(input_5, [batch_size, num_LSTMs_5, num_steps, size])
...
def main(_):
    ...
    # np.save('./sklt_npy_view_TPAMI/feature_train_view_all', feature_train)
    # np.save('./sklt_npy_view_TPAMI/AS_train_label_view_all', AS_train_label)
    # np.save('./sklt_npy_view_TPAMI/feature_test_view_all', feature_test)
    # np.save('./sklt_npy_view_TPAMI/AS_test_label_view_all', AS_test_label)
    # print 'Skeleton data stored.'

    # LOAD .npy: ALL
    feature_train = np.load('./sklt_npy_view_TPAMI/feature_train_{0}_all.npy'.format(view_subject))
    AS_train_label = np.load('./sklt_npy_view_TPAMI/AS_train_label_{0}_all.npy'.format(view_subject))
    feature_test = np.load('./sklt_npy_view_TPAMI/feature_test_{0}_all.npy'.format(view_subject))
    AS_test_label = np.load('./sklt_npy_view_TPAMI/AS_test_label_{0}_all.npy'.format(view_subject))
