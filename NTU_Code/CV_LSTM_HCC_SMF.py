...
def run_epoch(session, m, data, label, eval_op, verbose=False, is_training=True):
    ...
    for step, (x, y) in enumerate(Action_input.MSR_iterator(data, label, m.batch_size, m.feature_size,
        ...
        cost_L, cost, accuracy, state, p_l, g_l, _ = session.run([m.cost_L, m.cost, m.accuracy, m.final_state, m.pred_labels,
    ...
    return costs_L, costs, accuracys / sumsums, p_ls, g_ls
...
def main(_):
    ...
    if writeConfig_tocsv:
        ...
        csvWriter.writerow([])
        csvWriter.writerow(['LSTM+HCC+SMF'])
    ...
    with tf.Graph().as_default(), tf.Session(config=sessConfig) as session:
        with tf.device('/cpu:0'):
            ...
            for i in range(config.max_max_epoch):
                ...
                train_cost_L, train_cost, train_accuracy, tr_p_l, tr_g_l = run_epoch(
                ...
                if i % 100 == 0:
                    test_cost_L, test_cost, test_accuracy, te_p_l, te_g_l = run_epoch(session,
                                                                                                                mtest,
                                                                                                                feature_test,
                                                                                                                AS_test_label,
                    ...
                    # else:
                    #     print(len(te_g_l))
                    #     print(np.sum(np.equal(te_g_l_pre, te_g_l)))
