...
# win_size = [77, 103, 103, 35, 154, 154]
# stride = [74, 98, 98, 33, 145, 145]
# start_time = [4, 3, 5, 4, 2, 3]

# win_size = [77, 103, 103, 103, 154, 154, 154, 154, 35, 35]
# stride = [74, 98, 98, 98, 145, 145, 145, 145, 33, 33]
# start_time = [4, 3, 5, 4, 2, 3, 1, 6, 4, 3]
...
win_size = win_size_data[0:NP]
stride = stride_data[0:NP]
start_time = start_time_data[0:NP]
...
myKeepProb = 0.4
view_subject = 'view'
folder_path = 'Generalized_TS-LSTM_view'

myFolderPath = './TrialSaver/%s/%04d%02d%02d_%02d%02d_BD_%d_KB_%.2f_NP_%02d_NS_%02d'%(folder_path,_now.year,
                                                                           _now.month,
                                                                           _now.day,
                                                                           _now.hour,
                                                                           _now.minute,
...
class MP_runner(object):
    ...
    def __init__(self, is_training, config, labels, win_sizes, strides, start_times):
        ...
        for wi in range(len(win_sizes)):
            ...
            if is_training:
                sw_general = '/gpu:' + str(wi % numofGPU)
            else:
                sw_general = '/cpu:0'
...
def main(_):
    ...
    eval_config.batch_size = len(feature_test) / 12
    ...
    if writeConfig_tocsv:
        ...
        # csvWriter.writerow(['offset', offset])
        ...
        with tf.device('/cpu:0'):
            ...
            for i in range(config.max_max_epoch):
                ...
                if i % 100 == 0:
                    ...
                    with open(folder_path + "/view-test-AM-" + str(i) + ".csv", "wb") as csvfile:
                        csvwriter2 = csv.writer(csvfile)
                        for j in range(config.class_size):
                            csvwriter2.writerow(confusion_matrix[j])
                    with open(folder_path + "/view-test-GM-" + str(i) + ".csv", "wb") as csvfile_geo:
